...
import {Client} from "@chainlink/contracts-ccip/src/v0.8/ccip/libraries/Client.sol"; //Chainlink CCIP
import {CCIPReceiver} from "@chainlink/contracts-ccip/src/v0.8/ccip/applications/CCIPReceiver.sol"; //Chainlink CCIP

// import {console} from "forge-std/Script.sol"; 

contract dNFT is ERC721, ERC721URIStorage, VRFConsumerBaseV2, CCIPReceiver, Ownable {
    ...
    mapping(address => uint256) private addressToTokenId; //All the tokenId for an address (THIS IS NOT IMPLEMENT YET)
    ...
    event MessageReceived(
        ...
        address token, // The token address that was transferred. We put the address of the man
    ...
    */
    ...
    constructor(address initialOwner, address vrfCoordinator, uint64 subscriptionId, address dataFeedAdress, address router)
        ERC721("dNFT", "DFT")
    ...
    // =======================================CCIP PART=============================================
    function updateScoreFromLastCCIPMessage() public { //NEED TO BE CALL BY AUTOMATION log trigger (When MessageReceived is emit) (NOT IMPLEMENT YET)
        (, , address tokenAddress, uint256 amountToken) = getLastReceivedMessageDetails();
        uint256 tokenId = addressToTokenId(tokenAddress);
        uint256 raffleId = RaffleByTokenId(tokenId);
        if (amountToken % 2 == 0) { // DEPEND ON THE CHAIN. HERE for each 2 ETH we increment the score by 1 (Need to change after)
            updateScore(raffleId, tokenId, 1);
        }
    }
    // =======================================ENIGMA PART===========================================
    ...
    */
    ...
    function getLastReceivedMessageDetails()
        public
        view
        returns (bytes32 messageId, string memory text, address tokenAdress, uint256 amountToken)
    {
